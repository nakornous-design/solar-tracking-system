import { NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";
import { authorizeRequest } from "@/lib/api-permissions";
import { submitLoanFallbackDecision } from "@/services/workflow/loanFallbackEngine";

const supabaseAdmin = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
);

export async function POST(
  request: Request,
  { params }: { params: Promise<{ projectId: string }> },
) {
  try {
    const permission = await authorizeRequest(supabaseAdmin, request, ["admin", "supervisor", "sales", "finance", "ops", "rcm", "sbc"]);
    if (!permission.ok) return permission.response;

    const { projectId } = await params;
    const { projectStageId, action, reason, evidence = [] } = await request.json();

    if (!projectStageId) {
      return NextResponse.json({ error: "projectStageId is required." }, { status: 400 });
    }

    const result = await submitLoanFallbackDecision(
      supabaseAdmin,
      projectId,
      projectStageId,
      action,
      reason,
      evidence,
      permission.userId,
    );

    if (!result.ok) {
      return NextResponse.json(result, { status: result.status });
    }

    return NextResponse.json(result);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Unknown error";
    console.error("Loan fallback API Error:", message);
    return NextResponse.json({ error: `Loan fallback failed: ${message}` }, { status: 500 });
  }
}
