import { NextResponse } from "next/server";
import { createClient } from "@supabase/supabase-js";
import { scheduleProjectStage } from "@/services/workflow/resourceSchedulingEngine";
import { authorizeRequest } from "@/lib/api-permissions";

const supabaseAdmin = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!,
);

export async function POST(
  request: Request,
  { params }: { params: Promise<{ projectId: string }> },
) {
  try {
    const permission = await authorizeRequest(supabaseAdmin, request, ["admin", "supervisor", "ops", "sbc"]);
    if (!permission.ok) return permission.response;

    const { projectId } = await params;
    const body = await request.json();

    const result = await scheduleProjectStage(supabaseAdmin, {
      projectId,
      projectStageId: body.projectStageId,
      scheduledStart: body.scheduledStart,
      scheduledEnd: body.scheduledEnd,
      resourceTeamId: body.resourceTeamId,
      requiredSkill: body.requiredSkill,
      territory: body.territory,
      notes: body.notes,
      actorUserId: permission.userId,
    });

    if (!result.ok) {
      return NextResponse.json(result, { status: result.status });
    }

    return NextResponse.json(result);
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "Unknown error";
    console.error("Schedule API Error:", message);
    return NextResponse.json({ error: `Scheduling failed: ${message}` }, { status: 500 });
  }
}
